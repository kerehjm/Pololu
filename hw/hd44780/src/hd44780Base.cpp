...
void PololuHD44780Base::loadCustomCharacter(const uint8_t * picture, uint8_t number)
{
    ...
    for(uint8_t i = 0; i < 8; i++)
    {
        ...
        // sendData(pgm_read_byte(picture + i)); TODO 
    }
}
...
//private
void PololuHD44780Base::sendCommand(uint8_t cmd)
{
...
void PololuHD44780Base::sendCommand4Bit(uint8_t cmd)
{
    sendAndDelay(cmd, false, true);
}

void PololuHD44780Base::sendData(uint8_t data)
{
    sendAndDelay(data, true, false);
}

//public
void PololuHD44780Base::loadCustomCharacter(const char * picture, uint8_t number)
{
    loadCustomCharacter((const uint8_t *)picture, number);
}

void PololuHD44780Base::createChar(uint8_t number, uint8_t picture[])
{
    loadCustomCharacterFromRam(picture, number);
}

void PololuHD44780Base::setCursor(uint8_t col, uint8_t row)
{
    gotoXY(col, row);
}
